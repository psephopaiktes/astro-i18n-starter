...
Inside `src/components/i18n/`, you will find several UI components that are useful for generating multilingual sites. Feel free to customize their content or import them as needed.

### LocaleSelect.astro
A select UI for switching between languages.

### LocaleSelectSingle.astro
A link UI for switching between languages. Use this when you only support two languages.
...
Many deployment services use this file when a page is not found.
...
Using `history.replaceState()` ensures that the browser displays the pathname of the URL the user accessed before the redirect.
